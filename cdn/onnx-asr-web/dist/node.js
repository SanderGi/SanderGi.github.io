...
function normalizeRepoId(repoId) {
  const parts = repoId.split("/").map((part) => part.trim()).filter(Boolean);
  if (parts.length < 2) {
...
function modelFilenameCandidates(filename, quantization = "int8") {
  ...
  const unique = /* @__PURE__ */ new Set([dotInt8, underscoreInt8, filename]);
  return [...unique];
}
...
async function listHuggingFaceRepoFiles(repoId, revision, endpoint, requestOptions) {
  ...
  const files = payload.map((item) => item.path).filter((path) => typeof path === "string");
...
async function downloadPath(baseUrl, modelDir, relativePath, requestOptions, { required = true } = {}) {
  ...
  const response = required ? await fetchRequired(url, requestOptions) : await fetchOptional(url, requestOptions);
  if (!response) {
    return false;
  }
...
function selectSherpaArtifacts(files, quantization) {
  ...
  const tokens = files.includes("lang/tokens.txt") ? "lang/tokens.txt" : files.includes("tokens.txt") ? "tokens.txt" : null;
  if (!tokens) {
...
async function loadLocalVadModel(modelDir, options = {}) {
  const quantization = options.quantization ?? "int8";
  const modelPath = modelDir.endsWith(".onnx") ? modelDir : join(modelDir, selectSileroVadArtifact(await walkFiles(modelDir), quantization));
...
async function downloadHuggingfaceVadModel(repoId, options = {}) {
  ...
  const headers = hfToken ? { Authorization: `Bearer ${hfToken}` } : void 0;
...
async function loadLocalModel(modelDir, options = {}) {
  const quantization = options.quantization ?? "int8";
  const files = await walkFiles(modelDir);
  ...
  if (spec.decoderKind === "whisper-ort") {
    ...
    return createAsrModelWithVad({
      ...
      addedTokensJson: artifacts.addedTokensPath ? await readFile(join(modelDir, artifacts.addedTokensPath), "utf8") : "{}",
      sessionOptions: options.sessionOptions,
      decoderOptions: options.decoderOptions
    }, options);
  }
  if (spec.decoderKind === "whisper-hf") {
    const encoderFile2 = await selectOrFallbackModelFile(modelDir, spec.encoder, quantization);
    const decoderFile2 = await selectOrFallbackModelFile(modelDir, spec.decoderJoint, quantization);
    return createAsrModelWithVad({
      modelType,
      decoderKind: spec.decoderKind,
      config,
      encoderModel: join(modelDir, encoderFile2),
      decoderJointModel: join(modelDir, decoderFile2),
      vocabJson: await readFile(join(modelDir, "vocab.json"), "utf8"),
      addedTokensJson: await fileExists(join(modelDir, "added_tokens.json")) ? await readFile(join(modelDir, "added_tokens.json"), "utf8") : "{}",
      sessionOptions: options.sessionOptions,
      decoderOptions: options.decoderOptions
    }, options);
  }
  const encoderFile = await selectOrFallbackModelFile(modelDir, spec.encoder, quantization);
  const decoderFile = spec.decoderJoint ? await selectOrFallbackModelFile(modelDir, spec.decoderJoint, quantization) : null;
  const preprocessorFile = spec.preprocessor ? await selectOrFallbackModelFile(modelDir, spec.preprocessor, quantization) : null;
...
async function downloadHuggingfaceModel(repoId, options = {}) {
  const fetchImpl = options.fetch ?? globalThis.fetch;
  if (!fetchImpl) {
    throw new Error("No fetch implementation available.");
  }
  ...
  if (spec.decoderKind === "nemo-conformer") {
    ...
    const downloadList = artifacts.mode === "rnnt" ? [artifacts.encoder, artifacts.decoderJoint] : [artifacts.ctcModel];
  ...
  if (spec.decoderKind === "gigaam") {
    ...
    const downloadList = artifacts.mode === "rnnt" ? [artifacts.encoder, artifacts.decoder, artifacts.joint] : [artifacts.ctcModel];
    for (const file of downloadList) {
      if (options.forceDownload || !await fileExists(join(modelDir, file))) {
        await downloadPath(baseUrl, modelDir, file, requestOptions, { required: true });
      }
  ...
  if (spec.decoderKind === "whisper-hf") {
    const repoFiles = await listHuggingFaceRepoFiles(repoId, revision, options.endpoint, requestOptions);
    const modelFiles2 = [spec.encoder, spec.decoderJoint];
    for (const baseFile of modelFiles2) {
